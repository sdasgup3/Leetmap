...
public:
    ...
    void buildBST(TreeNode**root, int n) {
        if(NULL == *root) {
            *root = new TreeNode(n);
            (*root)->count = 1;
            return;
    ...
    vector<int> countSmaller1(vector<int>& nums) {
        ...
        for(int i = 0 ; i < sz; i++) {
            root = NULL;
            //Create a BST with nums[i] as root and with the nums of its right
            for(int j = i; j < sz; j++) {
                buildBST(&root, nums[j]);
            }

            //cout << root << "\n";
            //Extract the nums of elems on root's left
            if(root->left) {
                res.push_back(root->left->count);
            } else {
                res.push_back(0);
            }
        }
